...
STRATEGY_MODES = [
    # control ROI-mode
    "long_breakout_v5_rsi_high_V11",

    # control production-mode
    "long_breakout_v5_rsi_high_V14",

    # new ADX threshold tests
    "long_breakout_v5_rsi_high_V17",
    "long_breakout_v5_rsi_high_V18",
    "long_breakout_v5_rsi_high_V19",
    "long_breakout_v5_rsi_high_V20",
    "long_breakout_v5_rsi_high_V21",
]
...
def calculate_vol_peak_offset_10(volume_ratio: pd.Series) -> pd.Series:
    """
    vol_peak_offset_10:
    - window = last 11 bars: -10 ... 0
    - 0 means max volume_ratio is on signal bar
    - -1 means max was 1 bar ago
    - -4 means max was 4 bars ago
    """
...
def load_indicator_file(symbol: str, timeframe: str) -> pd.DataFrame | None:
    ...
    df["vol_peak_offset_10"] = calculate_vol_peak_offset_10(df[VOLUME_RATIO20_COL])
...
def simulate_exit_long(
    ...
    for i in range(entry_idx, len(open_times)):
        ...
        # якщо в одній 1m свічці був і SL, і TP — рахуємо SL першим.
        if low <= current_sl:
...
def build_base_v5_mask(df: pd.DataFrame) -> pd.Series:
    rsi14 = df[RSI14_COL]
...
def build_v11_mask(df: pd.DataFrame) -> pd.Series:
    base_v5 = build_base_v5_mask(df)

    close_pos = df[CLOSE_POS_COL]
    vol_peak_offset_10 = df["vol_peak_offset_10"]

    return (
        base_v5
        & vol_peak_offset_10.notna()
        & close_pos.notna()
        & (vol_peak_offset_10 >= -4)
        & (close_pos < 0.85)
    )


def build_entry_mask(df: pd.DataFrame, strategy_mode: str) -> pd.Series:
    v11 = build_v11_mask(df)
    ...
    # =========================
    # V11 — control ROI-mode
    # V5 + vol_peak_offset_10 >= -4 + close_position < 0.85
    # =========================
    if strategy_mode == "long_breakout_v5_rsi_high_V11":
        mask = v11

    # =========================
    # V14 — control production-mode
    # V11 + ADX14 >= 25
    # =========================
    elif strategy_mode == "long_breakout_v5_rsi_high_V14":
        mask = (
            v11
            & adx14.notna()
            & (adx14 >= 25)
    ...
    # V11 + ADX14 >= 20
    # =========================
    elif strategy_mode == "long_breakout_v5_rsi_high_V17":
        mask = (
            v11
            & adx14.notna()
            & (adx14 >= 20)
    ...
    # V11 + ADX14 >= 22
    # =========================
    elif strategy_mode == "long_breakout_v5_rsi_high_V18":
        mask = (
            v11
            & adx14.notna()
            & (adx14 >= 22)
    ...
    # V11 + ADX14 >= 27
    # =========================
    elif strategy_mode == "long_breakout_v5_rsi_high_V19":
        mask = (
            v11
            & adx14.notna()
            & (adx14 >= 27)
    ...
    # V11 + ADX14 22–45
    # =========================
    elif strategy_mode == "long_breakout_v5_rsi_high_V20":
        mask = (
            v11
            & adx14.notna()
            & (adx14 >= 22)
    ...
    # V11 + ADX14 25–45
    # =========================
    elif strategy_mode == "long_breakout_v5_rsi_high_V21":
        mask = (
            v11
            & adx14.notna()
            & (adx14 >= 25)
...
def collect_candidates_for_file(
    ...
    df = load_indicator_file(symbol=symbol, timeframe=timeframe)
...
def build_summary(
    ...
    grouped = trades_df.groupby(["symbol", "timeframe", "side"], dropna=False)
...
def run_single_backtest(strategy_mode: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    print("=" * 100)
    print(f"BACKTEST BEST: {strategy_mode.upper()}")
    ...
    trades_path = RESULTS_DIR / f"manual_backtest_best_{strategy_mode}_trades.csv"
    summary_path = RESULTS_DIR / f"manual_backtest_best_{strategy_mode}_summary.csv"
...
def run_all_backtests() -> None:
    ...
    compare_path = RESULTS_DIR / "manual_backtest_best_compare_summary.csv"
    compare_df.to_csv(compare_path, index=False)

    global_compare = compare_df[compare_df["scope"] == "GLOBAL"].copy()

    print("\n" + "=" * 100)
    print("BEST MODEL COMPARE SUMMARY")
    print("=" * 100)
